...
│   ├── requirements.txt      Dependencies (PyQt5, pyte)
...
└── android/                  Android project (open this directory in Android Studio)
    └── app/src/main/java/com/example/usbbridge/
        ├── MainActivity.kt        UI: service / Token / files / text
...
```bash
cd pc
pip install -r requirements.txt      # PyQt5 + pyte
```
...
### Smoke checklist (SSH hub)

1. Phone: add `ID_01`, Start, complete host trust / MFA on phone → state `running`.
2. PC: Connect USB bridge → Remote Terminal → Refresh → Attach `ID_01` → type `uname -a`, try arrow keys / Ctrl-C.
3. PC: Detach → session still running on phone → Attach again (backlog may replay).
4. Unplug USB → phone session still running → reconnect USB → Attach again.
5. Phone: Stop `ID_01` → PC shows session closed / cannot attach until Start again.
